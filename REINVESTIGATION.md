...
`plots/figures/` now holds **this** set and only this set — every file in it is post-fix,
regenerated by `plots/make_reinvestigation_figures.py` reading `workspace/` directly, in both
`.png` and `.pdf`. The figures of the original study have moved to `plots/figures_original_study/`,
which is what `FINDINGS.md` reads. The two sets describe different agents and must not be read
together; `plots/figures/PROVENANCE.md` records where each file comes from.
...
![Consolidation, before vs after the fixes](plots/figures/consolidation_prepost.png)

*The same three panels as the original study's `consolidation_internals`, redrawn post-fix — this
is what the fixes did to the mechanism itself. (a) the first, mean and last gradient step of each
cycle now separate, a ×1.31 descent where the old curves lay on top of each other (inert control:
×1.02). (c) `‖V_trans‖` after ÷ before is **0.950 = λ exactly**; the old `decay_mode="params"`
was deleting ~90 % of the transient where the algorithm asks for 5 %. The sawtooth survives the
fixes and should: the target is `old_V_perm + V_trans`, so the regression's difficulty is set by
how much the transient is carrying — `corr(loss, ‖V_trans‖₂) = 0.96` over the run.*

![Consolidation in situ, fitted vs held-out](plots/figures/consolidation_insitu.png)

*Job G. The transfer is measured separately on the states the regression fitted and on states it
never saw. The two curves lie on top of each other for the whole run, ratio **1.001** — the
permanent has learned a function, not a lookup table over its buffer.*

![Consolidation regression, per cycle](plots/figures/consolidation_loss_curves.png)
