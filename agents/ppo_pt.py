"""PT-PPO with permanent-transient actor AND critic decompositions — THE PT agent.

Anand & Precup (2023) decompose the value function; this extends the same decomposition to the
policy mean, since PPO is an actor-critic and a split critic alone leaves the policy with no slow
timescale. Four networks in total, two per component, with no shared trunk — the timescale
separation is the mechanism, and a shared trunk couples the two components through the fast
learner's gradients.

    critic:  V(s)  = V_P(s) + V_T(s)          (SplitCritic)
    actor:   mu(s) = mu_P(s) + mu_T(s)        (SplitGaussianActor; log_std is NOT split)

Transient (fast, Adam): trained every PPO update. Permanent (slow): trained only at consolidation,
every k updates, where it regresses toward `perm + rho*trans` over a rolling buffer of visited
states, after which the transient's OUTPUT LAYER is scaled by (1-rho). Absorb rho, retain 1-rho:
one transfer, composition-preserving, so behaviour does not jump. `decay_rho` can decouple the two
halves; it exists only to reproduce the measured divergence that follows.

This file was `ppo_pt_full.py` / `PPOPTFull` through Phase 1, when a legacy critic-only PT agent
held the plain name. That agent is archived at `archive/phase1/agents/ppo_pt_critic_only.py`.
"""
import copy
import time
...
class PPOPT(PPOBase):
    ...
    def __init__(self, obs_dim, act_dim, cfg, device):
        ...
        if self.ent_coef != 0.0:
            raise ValueError("pt requires ent_coef=0; use kl_prior_coef for regularization")
        ...
        # TASK ID TO THE TRANSIENT ONLY. When the env appends a one-hot task label,
        # `task_id_dims` tells the split nets to hide that tail from the PERMANENT: the permanent
        # learns what generalises across tasks, the transient reads the label and supplies the
        # task-specific correction. That is the division of labour in Anand & Precup's own control
        # experiment, where the transient reads the reward-correlated feature. 0 = both nets see
        # the whole observation (the behaviour of every run before this).
        # Derived HERE, from the same two keys the env uses, rather than read from a value injected
        # into cfg by train.py. That injection made the permanent's blindness depend on WHO built
        # the agent: anything constructing it from a config alone (tests, probes, the pre-flight
        # gate) got a permanent that could see the task label, while training got one that could
        # not — the same object behaving two ways, which is untestable by construction.
        task_id_dims = (len(cfg.get("tasks", [])) if cfg.get("task_id_obs", False) else 0)
        if not cfg.get("task_id_to_transient_only", True):
            task_id_dims = 0
        self.perm_obs_dim = obs_dim - task_id_dims
        self.actor = SplitGaussianActor(
            obs_dim,
            act_dim,
            hidden_sizes=hidden,
            trans_hidden_sizes=actor_trans_hidden,
            perm_obs_dim=self.perm_obs_dim,
            log_std_init=cfg.get("log_std_init", 0.0),
            # Read from the SAME key vanilla and ewc read, so the three arms cannot silently
            # diverge on exploration. See models/actor.py.
            freeze_log_std=bool(cfg.get("freeze_log_std", True)),
            # Same key vanilla and ewc read (ppo_base.py). LayerNorm never touches the output
            # layer, so `decay_transient` stays an exact scaling of mu_T and Theorem 1's
            # zero-init still gives mu_T(s) == 0 — see models/actor.py::mlp.
        ...
        # The transient mean, plus log_std IF it is trainable. Excluding a trainable log_std here
        # would make `freeze_log_std: false` a silent no-op on this agent while vanilla and ewc
        # honoured it — a config key one agent reads and another ignores, which is exactly the
        # failure mode that invalidated a 24-run sweep.
        trans_params = list(self.actor.trans_mean.parameters())
        if self.actor.log_std.requires_grad:
            trans_params.append(self.actor.log_std)
        self.actor_optim = torch.optim.Adam(trans_params, lr=lr_trans, eps=adam_eps)
        ...
        # Same quantities on states EXCLUDED from the regression; None unless
        # consolidation_holdout_frac > 0. See _consolidate — this is the check on whether the
        # permanent generalises or has merely memorised the consolidation buffer.
        self.last_absorbed_frac_holdout = None
        self.last_absorbed_align_holdout = None
        self.last_actor_absorbed_frac_holdout = None
        self.last_actor_absorbed_align_holdout = None
    ...
    def update(self, last_obs, last_done, update_idx):
        ...
        with torch.no_grad():
            ...
            mu_perm = self.actor.perm_forward(probe)
        ...
        return {
            ...
            # PT overrides update(), so it was silently dropping the per-dimension MINIMUM that
            # PPOBase reports. The mean hides a single collapsed action dimension, and `pt` is the
            # arm where that was suspected on HalfCheetah — so the one arm that most needed the
            # diagnostic was the only one not logging it. Harmless on cartpole (act_dim 1, so min
            # == mean); a real blind spot at act_dim 6.
    ...
    def _iter_indices(self, n, subset=None):
        ...
        `subset` restricts the regression to those buffer positions — used by the holdout
        diagnostic, which trains on part of the buffer and measures on the rest.
        """
        pool = np.arange(n) if subset is None else np.asarray(subset)
        m = len(pool)
        for _ in range(self.consolidation_epochs):
            order = (np.random.permutation(m) if self.consolidation_shuffle
                     else np.arange(m))
            for start in range(0, m, self.minibatch_size):
                yield pool[order[start:min(start + self.minibatch_size, m)]]
    ...
    def _consolidate(self):
        ...
        with torch.no_grad():
            ...
            old_mu_perm = self.actor.perm_forward(states)
        ...
        # ---- HOLDOUT: does the permanent GENERALISE, or has it memorised the buffer? ----
        # `absorbed_frac` on its own is in-distribution. A permanent that memorised the
        # consolidation buffer reports a healthy number while being wrong on the NEW states the
        # next rollout bootstraps from — worst right after a switch, when the state distribution
        # has just moved. Offline the target IS fittable (3.2% train error) while held-out error
        # floors at 38-44% (FINDINGS 6.3); this is the in-situ version of that measurement.
        #
        # consolidation_holdout_frac = 0 (the default) skips the split entirely, so the default
        # path does not touch the RNG stream and stays seed-comparable. With it enabled the split
        # draws from np.random, so those runs are NOT bit-comparable with default-path runs.
        holdout_frac = float(self.cfg.get("consolidation_holdout_frac", 0.0))
        train_idx = hold_idx = None
        if holdout_frac > 0.0 and n >= 4:
            perm = np.random.permutation(n)
            n_hold = max(int(n * holdout_frac), 1)
            hold_idx, train_idx = perm[:n_hold], perm[n_hold:]

        for indices in self._iter_indices(n, subset=train_idx):
            idx = torch.as_tensor(indices, dtype=torch.long, device=self.device)
            pred = self.critic.perm_forward(states[idx]).squeeze(-1)
            ...
            pred_mu = self.actor.perm_forward(states[idx])
        ...
        with torch.no_grad():
            ...
            new_mu_perm = self.actor.perm_forward(states)
        # Fitted states: the ones the regression actually trained on.
        fit = slice(None) if train_idx is None else torch.as_tensor(
            train_idx, dtype=torch.long, device=self.device)
        self.last_absorbed_frac, self.last_absorbed_align = self._absorption(
            old_v_perm[fit], new_v_perm[fit], old_v_trans[fit], self.rho
        )
        self.last_actor_absorbed_frac, self.last_actor_absorbed_align = self._absorption(
            old_mu_perm[fit], new_mu_perm[fit], old_mu_trans[fit], self.rho
        )
        # Held-out states: never seen by the regression. A healthy fitted number beside a
        # near-zero held-out one means the transfer was memorised, not learned.
        self.last_absorbed_frac_holdout = None
        self.last_absorbed_align_holdout = None
        self.last_actor_absorbed_frac_holdout = None
        self.last_actor_absorbed_align_holdout = None
        if hold_idx is not None:
            hold = torch.as_tensor(hold_idx, dtype=torch.long, device=self.device)
            self.last_absorbed_frac_holdout, self.last_absorbed_align_holdout = self._absorption(
                old_v_perm[hold], new_v_perm[hold], old_v_trans[hold], self.rho
            )
            (self.last_actor_absorbed_frac_holdout,
             self.last_actor_absorbed_align_holdout) = self._absorption(
                old_mu_perm[hold], new_mu_perm[hold], old_mu_trans[hold], self.rho
            )
        ...
        # `decay_rho` defaults to rho: the permanent absorbs rho and the transient retains
        # (1-rho), so the composed function is unchanged and behaviour does not jump. Setting it
        # separately DECOUPLES the two halves of one transfer.
        #
        ...
        # consolidation and the error can compound over many cycles. Watch for divergence.
        #
        # (Phase 1 ran arms that varied these independently and drew conclusions about which half
        # of the mechanism mattered. Those conclusions rested on a comparison the supervisors have
        # since set aside, so they are NOT repeated here as fact. The flag remains because the
        # question — is it the decomposition or the decay? — is still open, and `probe/decay_gain`
        # in train.py is the measurement that addresses it directly.)
        ...
        record = {
            ...
            "absorbed_frac_holdout": self.last_absorbed_frac_holdout,
            "absorbed_align_holdout": self.last_absorbed_align_holdout,
            "actor_absorbed_frac_holdout": self.last_actor_absorbed_frac_holdout,
            "actor_absorbed_align_holdout": self.last_actor_absorbed_align_holdout,
    ...
    def measure_post_consolidation_drift(self, states_np):
        ...
        with torch.no_grad():
            ...
            old_mu = actor_before.perm_forward(states) + actor_before.trans_mean(states)
            new_mu = self.actor.perm_forward(states) + self.actor.trans_mean(states)
